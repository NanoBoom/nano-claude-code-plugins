...
description: Traces how a specific behavior works through control flow, data flow, state ownership, boundaries, and side effects with precise file:line evidence. Use for the HOW part of a codebase question, for mapping integration points before a change, or as a research area of `/boom:codebase-question`. Advisory only: read-only, does not modify files or commit.
