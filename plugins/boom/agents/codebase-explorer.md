...
description: Locates where a concern lives and returns the concrete files, precedents, primitives, tests, and validation commands that describe the codebase as it exists. Use for the WHERE part of a codebase question, for finding existing patterns and conventions before a change, or as a research area of `/boom:codebase-question`. Advisory only: read-only, does not modify files or commit.
