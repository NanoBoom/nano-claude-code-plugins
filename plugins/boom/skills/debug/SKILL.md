...
Spawn `boom:root-cause-analyzer` with an explicit `model` (sonnet by default; opus when the failure spans unfamiliar domains or concurrency), the original report, complete tracker context, repository path, and any decisive runtime evidence already available. Do not give it a preferred cause or fix.
