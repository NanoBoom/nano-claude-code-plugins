---
name: commit
description: Creates Git commits for completed work. Always use when committing changes, when the user explicitly asks to commit the work, when another workflow reaches its commit step, or when the user invokes /boom:commit.
...
**Bad:** `refactor(pr): update skill instructions`

**Good:** `refactor(pr): PR creation now uses one focused workflow`

## Commit and verify

Staging and committing require the user's explicit authorization in the current conversation. Invoking `/boom:commit` directly is that authorization; when another workflow reaches its commit step, confirm the authorization exists before staging. Stage and commit only the intended work. Verify the resulting commit contains every intended change, excludes unrelated changes, and leaves the remaining worktree state untouched. Do not amend or push unless explicitly requested.
