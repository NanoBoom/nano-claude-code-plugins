---
name: pr
description: Creates and opens GitHub pull requests. Always use when creating a PR on GitHub, when the user explicitly asks to create or open a PR, when another workflow reaches its PR-creation step, or when the user invokes /boom:pr.
...
Run this at the PR-creation step after the requested work has been done by the current agent, a subagent, or another collaborator. Every change belonging to that work must be committed before opening the PR. If intended changes remain uncommitted, stop and list them: staging and committing require the user's explicit authorization in the current conversation, and this skill does not carry it. Once authorized, invoke `/boom:commit` with a natural-language target that identifies only those changes, then verify the resulting commit. Never sweep unrelated worktree changes into the commit.
