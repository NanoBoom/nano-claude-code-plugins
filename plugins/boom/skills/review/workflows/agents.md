...
including when the review stops early. The canonical report lives under `$REVIEW_HOME` and outlives it.
...
- Read the previous canonical review under `$REVIEW_HOME` when it exists. On re-review, read every
  recorded finding disposition and its evidence.
...
| Scope | Agent | Focus |
|---|---|---|
| `code` | `boom:reviewer` | General correctness, sanity, scope, and repository fit |
| `seams` | `boom:seam-analyzer` | Missing types, counterpart drift, bypassed boundaries |
| `tests` | `boom:pr-test-analyzer` | Behavioral coverage and valuable regression protection |
| `comments` | `boom:comment-analyzer` | Accuracy and long-term value of changed comments |
| `errors` | `boom:silent-failure-hunter` | Swallowed failures, fallbacks, and actionable errors |
| `docs` | `boom:docs-impact-agent` | Stale or missing user and contributor documentation |
| `simplify` | `boom:code-simplifier` | Premature machinery and smaller coherent structures |

`all` selects every scope.

## 4. Launch reviewers

Every selected scope is one worker start under `boom:dispatch-policy`: the three default scopes fit
L2, and `all` needs L3 or L4. Dispatch with `subagent_type: boom:<agent>` and an explicit
`model: sonnet` (opus within the allowed set when the diff touches permissions, data, concurrency,
migration, or public contracts). Run in parallel up to the level's concurrency cap, sequentially
beyond it. Every selected role remains required; wait for all of them before aggregation.
...
Persist what each reviewer returns. This review's round is `1` when `$REVIEW_HOME/reviews/pr-<number>/`
holds no `round-*` directory, and one higher than the largest otherwise. Create
`$REVIEW_HOME/reviews/pr-<number>/round-<n>/` and write every reviewer's report to `<scope>.md` there,
...
Write the report to the expanded absolute path `$REVIEW_HOME/reviews/pr-{NUMBER}-review.md`, then copy it
to `$REVIEW_HOME/reviews/pr-{NUMBER}/round-{n}/report.md`. The canonical path always holds the current
...
`boom-review-id` marker—edit that exact comment by ID; otherwise create it with `gh pr comment`. Do not
...
Append this round to `$REVIEW_HOME/reviews/rounds.jsonl`, creating the file when absent, as one JSON
