...
description: Research codebase questions using parallel agents - documents what exists, not what should change. Use when the user asks how the codebase works, where something lives, or invokes /boom:codebase-question.
...
Agent dispatch is governed by the `boom:dispatch-policy` skill: load it before the first launch and keep one cumulative ledger for this task.

- Each research area is one agent start. Two or three areas are L2; four or five are L3.
- Every Agent call must pass an explicit `model`. The hook denies a dispatch that omits one.
- Default `sonnet` for `boom:codebase-explorer` and `boom:codebase-analyst`, `haiku` for `boom:web-researcher`. Drop `codebase-explorer` to `haiku` for a single narrow lookup; raise it to `opus` only when an area spans unfamiliar cross-module architecture, and `web-researcher` to `sonnet` when a claim needs cross-checking or version judgment.
- Answer directly at L0 when the question is already resolvable from the current context or a single file read. Do not spawn an agent to confirm what you already have evidence for.
...
|------|-----------|-------------|
| **Where** | "where is", "find", "locate" | `boom:codebase-explorer` primary |
| **How** | "how does", "trace", "flow" | `boom:codebase-analyst` primary |
| **What** | "what is", "explain", "describe" | Both agents in parallel |
| **Pattern** | "how do we", "convention", "examples" | `boom:codebase-explorer` primary |
| **External** | "docs", "best practice", "API" | Add `boom:web-researcher` |
...
AREAS:
1. {Area} → Agent: {which agent} (model: {alias})
2. {Area} → Agent: {which agent} (model: {alias})
3. {Area} → Agent: {which agent} (model: {alias})
```
...
| Agent | Use When |
|-------|----------|
| `boom:codebase-explorer` | Finding WHERE code lives, locating files, extracting patterns, discovering conventions |
| `boom:codebase-analyst` | Understanding HOW code works, tracing data flow, mapping integration points |
| `boom:web-researcher` | Only when `--web` flag is set or user explicitly asks for external docs |

**Strategy:**
1. Start with `boom:codebase-explorer` to find what exists
2. Then use `boom:codebase-analyst` on the most relevant findings to trace how they work
3. Run agents in parallel when they're searching for different areas

**PHASE_2_CHECKPOINT:**
- [ ] Query decomposed into 2-5 research areas
- [ ] Agent and explicit model assigned to each area
- [ ] Task level declared and within its start budget
...
**Launch agents in parallel using multiple Agent tool calls in a single message, each with an explicit `subagent_type` and `model`.**

For each research area, use the appropriate agent:

**`boom:codebase-explorer`:**
...
**PHASE_3_CHECKPOINT:**
- [ ] All agents launched with explicit model (parallel where possible)
...
Research documents live outside the repository so they survive worktree cleanup and never land in a commit:

```bash
# --- Research store resolver ---
...
RESEARCH_HOME="${_hit%/project.json}"
[ -n "$RESEARCH_HOME" ] || RESEARCH_HOME="$_home/${_name:-project}-$(printf %s "$_root" | git hash-object --stdin | cut -c1-8)"
mkdir -p "$RESEARCH_HOME"; [ -f "$RESEARCH_HOME/project.json" ] || printf '{"path": "%s", "name": "%s"}\n' "$_root" "${_name:-project}" > "$RESEARCH_HOME/project.json"
mkdir -p "$RESEARCH_HOME/research"
```
...
**Path**: `$RESEARCH_HOME/research/{YYYY-MM-DD}-{kebab-case-topic}.md`
...
4. Spawn new agents as needed, each within the remaining start budget
...
**Document**: `{expanded absolute path to $RESEARCH_HOME/research/{filename}.md}`
...
To dig deeper: `/boom:codebase-question --follow-up {topic}`
To include external docs: `/boom:codebase-question --web {topic}`
```
...
/boom:codebase-question how does authentication work

# Include external documentation
/boom:codebase-question --web how does the dispatch hook validate a workflow script

# Follow up on previous research
/boom:codebase-question --follow-up what error handling patterns exist in the hook

# Locate and document a specific area
/boom:codebase-question where are all the agent definitions and how are they structured
```
...
3. **Agents are parallel, budgeted, and explicitly routed.** Launch multiple agents simultaneously when researching different areas, each naming its own model, and stop at the task level's start limit.
...
- **AGENTS_USED**: Specialized agents spawned for each research area, each with an explicit model
- **BUDGET_RESPECTED**: Starts stayed within the declared task level
- **EVIDENCE_COMPLETE**: Every finding has `file:line` references
- **DOCUMENT_CREATED**: Research file saved at `{expanded absolute path to $RESEARCH_HOME/research/}`
